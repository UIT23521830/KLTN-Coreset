...
class M03_CoreTabSelector(BaseCoresetSelector):
    ...
    def select(self, X_train: np.ndarray, y_train: np.ndarray, budget_ratio: float) -> np.ndarray:
        ...
        # Dùng min_samples_leaf=10000 để giới hạn cây theo yêu cầu (không dùng max_depth)
        dt_params = {'min_samples_leaf': 10000}
        coretab = CoreTabDT(sample_percent=0.15, examples_to_keep=1, params=dt_params)
